...
export default function TipJar() {
  ...
  const JarScriptAddress = "addr_test1qrnen5pw7yatfz5s95kn8pdfrx97v6z4leymzaahllde8zcn08geeynhynw2j3udpef5ryep0crcx6acmek7769ttsxsq5t2ah"; 
  ...
  async function sendTip() {
    try {
      ...
      tx.sendLovelace(JarScriptAddress, '5000000');
      //const builtTx = await tx.build(); // Build before attaching metadata
      // https://meshjs.dev/apis/transaction/basics#metadata
      //builtTx.addMetadata(721, { purpose: "Aiken Smart Contract Tip" });
      tx.setMetadata(0, 'Aiken Smart Contract Tip');

      const unsignedTx = await tx.build();
      const signedTx = await wallet.signTx(unsignedTx);
      const submittedTx = await wallet.submitTx(signedTx);
      setMessage(`Transaction submitted: ${submittedTx}`);
    } catch (error) {
...
<p>Go to the <a href="/five_tipjar_wallet" className="text-sky-600">Withdraw from Tips</a> page.</p>
      </main>
    </div>
  );
}